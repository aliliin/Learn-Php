        ...
        while (1) {
            ...
            while (1) {
                ...
                if ($pos === false) {
                    ...
                    $recv .= trim(substr($buffer, 0, $pos + 1)); // 处理收到的消息

                    // 如果服务器定义了消息处理回调函数，则在当前连接上将消息传入回调函数并执行回调
                    if ($this->onMessage) {
                        call_user_func($this->onMessage, $conn, $recv);
                    }

                    // 如果接收到 quit 消息，表示关闭词连接 ，等待下一个客户端连接
                    if ($recv == 'quit') {
...
// 启动服务器主进程
$server->run();
// 由于我们设置主进程以后台守护进程方式运行，所以启动成功之后就会将控制权交回给操作系统，我们可以通过 ps -ef | grep php 命令查看 TCP 服务器的运行情况